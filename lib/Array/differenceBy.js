...
function _differenceBy(array, value, fn) {
	...
	if (fn) {
		console.log('11')
		if (isFunction(fn)) {
			let newArray = array.map(item => fn(item))
			let newValue = value.map(item => fn(item))
			console.log('22')
			_difference(newArray, newValue)
		} else {
			//
		}
	} else {
		_difference(array, value)
	}
}

// console.log(_.differenceBy([3.1, 2.2, 1.3], [4.4, 1.3], Math.floor))
console.log(_differenceBy([3.1, 2.2, 1.3], [4.4, 1.3], Math.floor))
